...
and return null for empty or all-null input. `SumAsync`, `MinAsync`, and `MaxAsync` use
`IGwQueryExecutor.ReduceAsync`, which reads one provider-produced scalar row and cannot silently
