...
| `GW-QUERY-032` | A declared-reference join reached a provider or execution path whose join renderer has not landed yet (currently Mongo, in-memory, and batched query execution); the request fails closed instead of executing only its driving table |
