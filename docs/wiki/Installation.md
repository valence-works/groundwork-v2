...
```bash
gh workflow run publish-nuget.yml --ref v0.4.0-preview.14 \
  -f version=0.4.0-preview.14 -f publish=true -f confirm=0.4.0-preview.14
```
...
dotnet add package Groundwork.Records.Store --version 0.4.0-preview.14
```
...
```bash
dotnet tool install --global Groundwork.Tool --version 0.4.0-preview.14 \
