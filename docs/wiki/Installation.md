...
```bash
gh workflow run publish-nuget.yml --ref v0.4.0-preview.20 \
  -f version=0.4.0-preview.20 -f publish=true -f confirm=0.4.0-preview.20
```
...
```bash
dotnet add package Groundwork.Sqlite --version 0.4.0-preview.20
dotnet add package Groundwork.Records.Store --version 0.4.0-preview.20
```
...
```bash
dotnet tool install --global Groundwork.Tool --version 0.4.0-preview.20 \
