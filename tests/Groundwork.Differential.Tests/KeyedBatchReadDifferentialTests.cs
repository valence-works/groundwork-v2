...
public sealed class KeyedBatchReadDifferentialTests
{
    ...
    public async Task Batch_read_beyond_the_thousand_value_cap_matches_identically_on_every_provider()
    {
        ...
        // Every second key plus a run of keys past the end of the table. This is 1,300 distinct
        // requested keys, so all four providers must exercise the shared chunking path while the
        // assertion still covers gaps inside the key range and missing keys beyond it.
        var requested = Enumerable.Range(1, RowCount)
            .Where(id => id % 2 == 0)
            .Concat(Enumerable.Range(RowCount + 1, 50))
            .Select(id => (object?)(long)id)
            .ToArray();
        Assert.True(requested.Length > 1_000);
...
internal sealed class KeyedBatchReadMatrix : IDisposable
{
    ...
    private readonly LivePostgreSqlStore postgres;

    private KeyedBatchReadMatrix(string tableName, LivePostgreSqlStore postgres)
    {
        TableName = tableName;
    ...
    {
        var postgres = LivePostgreSqlStore.OpenOrSkip();
        var matrix = new KeyedBatchReadMatrix("g2_batch_read_" + Guid.NewGuid().ToString("N"), postgres);
        try
        {
            ...
            matrix.Add("PostgreSQL", new PostgreSqlProviderFactory().Create(postgres.ConnectionString));
