...
public sealed class CoverageAnalyzerTests
{
    ...
    [Fact]
    public async Task Linq_analyzer_refuses_a_second_declared_reference_join()
    {
        const string source = """
            using Groundwork.Query.Linq;
            using Groundwork.Query.Model;
            ...
            public static class Use
            {
                    ...
                    GwTableModel<Customer> customers, ReferenceJoin first, ReferenceJoin second)
                {
                    ...
                    database.Table(orders).Join(customer).Join(otherCustomer)
                        .Where(order => order.OtherCustomer.Name == "Ada");
                }
            }
            """;
    ...
    [Fact]
    public async Task Linq_analyzer_resolves_an_unconditional_declared_join_reassignment()
    {
        const string source = """
            using Groundwork.Query.Linq;
            using Groundwork.Query.Model;
    ...
    [Fact]
    public async Task Linq_analyzer_fails_closed_for_a_conditional_join_reassignment()
    {
        const string source = """
            using Groundwork.Query.Linq;
            using Groundwork.Query.Model;
            ...
            public static class Use
            {
                    ...
                    GwTableModel<Customer> customers, ReferenceJoin declaration)
                {
                    var reference = orders.Reference(order => order.Customer, customers, declaration);
                    var query = database.Table(orders);
                    if (enabled) query = query.Join(reference);
                    query.Where(order => order.Customer.Name == "Ada");
                }
            }
            """;
